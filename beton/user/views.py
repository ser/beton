...
def add_text():
    """Add a text banner."""
    # TODO: fg bg colours selection in the form

    def create_image_with_text(size, text, width, height, bg, fg):
        img = Image.new('RGB', (width, height), bg)
        draw = ImageDraw.Draw(img)
        draw.text((size[0], size[1]), text, font=fnt, fill=fg)
        return img

    def roll(text, width, height, bg, fg):
        for i in range(len(text)+1):
            new_frame = create_image_with_text((0,0), text[:i], width, height, bg, fg)
            frames.append(new_frame)

    frames = []
    all_text = []
    imgdir = current_app.config.get('UPLOADED_IMAGES_DEST') + '/tmp'
    form = AddBannerTextForm()
    if request.method == 'POST':
        if form.validate_on_submit():
            if form.banner_submit_save.data is True:
                log.debug(form.banner_tmpimage.data)
                filename = form.banner_tmpimage.data
                shutil.copy2(imgdir + '/' + filename, current_app.config.get('UPLOADED_IMAGES_DEST') + '/' + filename)
                Banner.create(
                    filename=filename,
                    owner=current_user.id,
                    url=form.banner_url.data,
                ...
                dblogger(
                    current_user.id,
                    "Text banner %s created successfully." % filename
                )
                flash('Banner added sucessfully.', 'success')
                return redirect(url_for('user.bannerz'))
            else:
                ...
                if form.banner_content_line3.data:
                    all_text.append(form.banner_content_line3.data)
                if form.banner_content_line4.data:
                    all_text.append(form.banner_content_line4.data)
                if form.banner_content_line5.data:
                ...
                tmpimage = tmpf.name.split("/")[-1]
                frames[0].save(tmpf, format='GIF',
                append_images=frames[1:], save_all=True, duration=180, loop=0)
                return render_template('users/text_banner.html',
                                        form=form,
                                        tmpimage=tmpimage)
        else:
            flash_errors(form)
